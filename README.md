...
Garanta que o GoLang está devidamente instalado e configurado na sua máquina. Dentro da pasta do seu projeto, crie uma subpasta com nome de sua preferência (no meu caso, criei com nome `graphql-server`)
...
Após implementar seu Schema no arquivo [schema.graphqls](https://github.com/stlandre/Backend-StudioG/blob/main/graphql-server/graph/schema.graphqls), certifique que o arquivo [resolver.go](https://github.com/stlandre/Backend-StudioG/blob/main/graphql-server/graph/resolver.go#L3) tenha a linha `//go:generate go run github.com/99designs/gqlgen`. Após isso, delete o arquivo `schema.resolvers.go` e rode os comandos
...
Além de requisições desse tipo, podemos adicionar o atributo `sublist` no corpo da requisição para obtermos a sub-lista de maior soma. Não era necessário, mas achei interessante colocar essa disponibilidade de informação, caso seja interesse do cliente:
